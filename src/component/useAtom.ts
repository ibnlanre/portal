 ...
 * @template Variables The type of the atom's `run` function.
 ...
 *
 * @param {Atom<State, Variables, Data, Context>} store The atom to use.
...
export function useAtom<
  ...
  Status
>(
  store: Atom<State, Variables, Data, Context, Status>,
  ...vars: Variables
  ...
  useShallowEffect(() => store.await(vars), vars);
  useEffect(() => {
    const subscriber = subscribe(setState);
    return subscriber.unsubscribe;
  }, []);

  const atom = get(state, ...vars);
