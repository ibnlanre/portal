import type { Assign } from "@/create-store/types/assign";
...
>(target: Target, source?: Source, cache = new WeakMap()) {
  if (Array.isArray(source)) {
    ...
    for (let index = 0; index < source.length; index++) {
      result = assign(result, source[index], cache);
    }
    return result;
  }
  ...
  assignHelper(result, properties, source, cache);
  assignHelper(result, symbols, source, cache);
 ...
 * If it is a dictionary, it is assigned with the source value.
 *
...
export function assignHelper(
  ...
  for (let index = 0; index < keys.length; index++) {
    ...
    if (isDictionary(targetValue) && isDictionary(sourceValue)) {
      result[key] = assign(targetValue, sourceValue, cache);
